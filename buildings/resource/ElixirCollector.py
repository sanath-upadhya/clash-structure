...
class ElixirCollector(ResourceBuilding):
    ...
    def __init__(self, number):
        picture = "elixir collector"
        self.current_level = 1
        cost = PurchaseCost(en.GE_COLLECTOR_BUILD_COST[self.current_level], en.GOLD)
        time = en.GE_COLLECTOR_BUILD_TIME[self.current_level]
        size = Size(2,2)
        hitpoints = en.GE_COLLECTOR_HIT_POINTS[self.current_level]
        type_of_resource = en.ELIXIR
    ...
    def upgrade_elixir_collector(self):
        super().upgrade_resource_building()

    def purchase_elixir_collector(self, position):
